...
const JobEdit = () => {
    ...
    // Fetch job data và dropdown options
    useEffect(() => {
        fetchJobData();
    ...
    const fetchJobData = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:3001/api/employer/jobs/${id}`,
                {
                    headers: {
            ...
            // ✅ SỬA: Parse salary range tốt hơn
            let salaryMin = "";
            let salaryMax = "";

            if (job.salary_range && job.salary_range !== "Thỏa thuận") {
                // Xử lý các format khác nhau: "5-8", "Từ 5 triệu đến 8 triệu", etc.
                ...
                if (numbers && numbers.length >= 2) {
                    ...
                    // Nếu là triệu thì nhân với 1,000,000
                    if (salaryStr.includes("triệu")) {
                        salaryMin = (parseInt(salaryMin) * 1000000).toString();
                        salaryMax = (parseInt(salaryMax) * 1000000).toString();
                    }
            ...
            setFormData({
                ...
                location_id: job.location_id?._id || "",
                position_id: job.position_id?._id || "",
                experience_id: job.experience_id?._id || "",
                education_id: job.education_id?._id || "",
                form_of_employment_id: job.form_of_employment_id?._id || "",
                salary_range_min: salaryMin,
                salary_range_max: salaryMax,
                quantity: job.quantity || 1,
                job_description:
                    typeof job.job_description === "object"
                        ? job.job_description.content || ""
                        : job.job_description || "",
            ...
            console.error("Lỗi khi lấy thông tin tin tuyển dụng:", error);
